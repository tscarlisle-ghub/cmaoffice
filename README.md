...
**04 · Miscellaneous Expenses** — moving expenses and any other one-off cost paid directly, outside all three contracts (a mover, a permit fee paid out of pocket, anything that doesn't belong to TCC, GHT, or Emory). Same one-click **Paid** checkbox pattern as Emory. This section is intentionally separate from the Overview's Contract Value / Paid to Date / Remaining KPIs at the top of the page, since those track the three vendor contracts specifically — Miscellaneous Expenses has its own small Total Logged / Paid to Date band instead. Say the word if you'd rather have it folded into the Overview totals.
...
**Sharing GitHub Sync with your partner** — once your own Repo Owner/Repo Name/Branch/File Path/Token fields are saved, **Copy 1-Click Link for Partner** bundles all five into one link and copies it. Send that link to your partner however you'd normally share something private (Messages, a password manager, in person) — when they open it, the app fills in and connects GitHub Sync automatically, no typing or extra clicks. The token briefly rides along in the link's address bar the moment it's opened, then the app immediately rewrites the address bar back to the plain URL, so it doesn't linger in their browser history. If you'd rather send plain text instead of a link, **Copy Setup as Text Instead** copies the same five fields as a block your partner can paste into their own copy via **Paste a Setup They Sent You** (then they still click **Save GitHub Settings** themselves). Either way, this is sharing your actual token with your partner — anyone who has the link or the text block can push and pull this repo, so treat it the way you'd treat a password.

Note that simply visiting the published GitHub Pages link — with no GitHub Sync setup at all — already loads the current `ledger-data.json` for read-only viewing. The link/text-block sharing above is specifically for giving your partner **write** access (so their edits push back to GitHub too), not for viewing.
